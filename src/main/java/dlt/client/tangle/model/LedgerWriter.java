  ...
  public void run() {
    ...
    while (!this.DLTOutboundMonitor.isInterrupted()) {
      try {
        ...
        if (transaction.getType().equals(TransactionType.LB_STATUS)) {
          String[] temp = transaction.getSource().split("/");

          tag = temp[2];
        } else {
          tag = transaction.getGroup();
        }

        this.writeToTangle(tag, transactionJson);
